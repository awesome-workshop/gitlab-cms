...
> Bear in mind that it is always a good idea to keep your analysis code under version control.
...
> You can request more datasets to be stored in `/eos/cms/store/group/cat` by creating an MR to [https://gitlab.cern.ch/cms-analysis/services/ci-dataset-files/-/blob/master/datasets.txt](https://gitlab.cern.ch/cms-analysis/services/ci-dataset-files/-/blob/master/datasets.txt).
{: .callout}

> ## Exercise: Set up a CI job that copies a file using the CAT EOS file service
>
> There is a few technical aspects that are involved in this.
> First, your GitLab CI job needs to be configured so that it creates an authentication token.
...
> ~~~
> Where `EOSPATH` is a variable holding the path to a file on EOS.
...
>
> There are a few technical aspects that are involved in this.
> First, your GitLab CI job needs to be configured so that it creates an authentication token.
> This is achieved with the following lines:
> ~~~
> id_tokens:
